<template>
  <form class="space-y-4" @reset.prevent="reset()" @submit.prevent="submit()">
    ...
    <UniqueId as="div" v-slot="{ id }" class="space-y-1 *:block">
      <Label :for="id">{{ t('labels.nonSeparatorTokens') }}</Label>
      <Textarea class="h-72 w-full" v-model="editableNonSeparatorTokens" />
    </UniqueId>
...
en:
  ...
  labels:
    nonSeparatorTokens: Non-Separator Tokens, separated by new lines
  toasts:
    success:
      title: Updating Non-Separator Tokens
      pendingText: Please wait...
      doneText: Done.
    error:
      title: Error while updating Non-Separator Tokens
      canceledText: Task was canceled.
      failedText: Task failed.
  confirmations:
    submit: Do you want to update your Non-Separator Tokens?
</i18n>
