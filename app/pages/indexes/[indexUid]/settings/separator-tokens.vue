<template>
  <form class="space-y-4" @reset.prevent="reset()" @submit.prevent="submit()">
    ...
    <UniqueId as="div" v-slot="{ id }" class="space-y-1 *:block">
      <Label :for="id">{{ t('labels.separatorTokens') }}</Label>
      <Textarea class="h-72 w-full" v-model="editableSeparatorTokens" />
    </UniqueId>
...
const submit = async () => {
  ...
  await handle(async () => {
    toast.spawn()
    await processTask(() => index.updateSeparatorTokens(self.separatorTokens), {
      onSuccess: async () => {
        toast.update({ ...TOAST_SUCCESS(t) })
        reset(self.separatorTokens)
      },
      onCanceled: () =>
        toast.update({
          ...TOAST_FAILURE(t),
...
en:
  ...
  labels:
    separatorTokens: Separator Tokens, separated by new lines
  toasts:
    success:
      title: Updating Separator Tokens
      pendingText: Please wait...
      doneText: Done.
    error:
      title: Error while updating Separator Tokens
      canceledText: Task was canceled.
      failedText: Task failed.
  confirmations:
    submit: Do you want to update your Separator Tokens?
</i18n>
