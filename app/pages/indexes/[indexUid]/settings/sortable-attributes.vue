<template>
  <form class="space-y-4" @reset.prevent="reset()" @submit.prevent="submit()">
    <h3 class="inline-flex w-full items-center justify-between text-xl font-semibold">
      {{ t('title') }}
      <DocumentationLink href="https://www.meilisearch.com/docs/reference/api/settings#sortable-attributes" />
    </h3>
    ...
    <UniqueId as="div" v-slot="{ id }" class="space-y-1 *:block">
      <Label :for="id">{{ t('labels.sortableAttributes') }}</Label>
      <Textarea class="h-72 w-full" v-model="editableSortableAttributes" />
    </UniqueId>
...
const { value: sortableAttributes, reset, modified } = resettableRef(await index.getSortableAttributes())
const { loading, error, handle } = useFormSubmit({
  confirm: { text: t('confirmations.submit') },
})
const processTask = useTask()
const { createToast } = useToasts()
const self = reactive({ sortableAttributes })
const editableSortableAttributes = computed({
  get: () => self.sortableAttributes.join('\n'),
  set: (value) =>
    (self.sortableAttributes = value
      .split('\n')
...
const submit = async () => {
  ...
  await handle(async () => {
    toast.spawn()
    await processTask(() => index.updateSortableAttributes(self.sortableAttributes), {
      onSuccess: async () => {
        toast.update({ ...TOAST_SUCCESS(t) })
        reset(self.sortableAttributes)
      },
      onCanceled: () =>
        toast.update({
          ...TOAST_FAILURE(t),
...
en:
  ...
  labels:
    sortableAttributes: Sortable attributes, separated by new lines
  toasts:
    success:
      title: Updating sortable attributes
      pendingText: Please wait...
      doneText: Done.
    error:
      title: Error while updating sortable attributes
  ...
  notice:
    title: Caution
    text: Updating sortable attributes will re-index all documents in the index, which can take some time.
