<template>
  <form class="space-y-4" @reset.prevent="reset()" @submit.prevent="submit()">
    <h3 class="inline-flex w-full items-center justify-between text-xl font-semibold">
      {{ t('title') }}
      <DocumentationLink href="https://www.meilisearch.com/docs/reference/api/settings#searchable-attributes" />
    </h3>
...
const { value: searchableAttributes, reset, modified } = resettableRef(await index.getSearchableAttributes())
...
const submit = async () => {
  ...
  await handle(async () => {
    toast.spawn()
    await processTask(() => index.updateSearchableAttributes(self.searchableAttributes), {
      onSuccess: async () => {
