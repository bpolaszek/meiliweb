...
export const exportDocuments = (client: Meilisearch, index: string, params: DocumentsQuery = {}) => {
  const retriever = async (offset: number, limit: number) => {
    const documents = await client.index(index).getDocuments({ offset, limit, ...params })
    return {
